...
1. 자기 서술적인 코드가 된다
   ...
   if (user.name.length < 2) throw "이름의 길이는 2글자 초과여야 합니다"; // 멀리 떨어져있는 곳에 이런 로직이 구현되어있으면 굉장히 당황스러울 수 있다
   ...
   class UserName { // 값 객체
      ...
      constructor(value: string) {
         if (value.name.length < 2) throw "이름의 길이는 2글자 초과여야 합니다";
...
2. 도메인에 변경사항이 있을 시 코드에 반영하기 쉽다
   ```typescript
   if (user.name.length < 3) throw "이름의 길이는 2글자 초과여야 합니다"; // 멀리 떨어져있는 곳을 찾아서 2 -> 3으로 바꾸기란 쉬운 작업이 아니다, 한 곳이 아니라 여러 곳이라면 상황은 더 심각해진다

   class UserName { // 값 객체
      constructor(value: string) {
         if (value.name.length < 3) throw "이름의 길이는 3글자 초과여야 합니다"; // 도메인 모델의 규칙이 기술된 곳이 명확하기 때문에 수정하기 쉽다
      }
   }
   ```
